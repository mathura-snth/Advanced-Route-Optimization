        ...
        if 'highway' in w.tags and w.tags['highway'] in self.valid_highways:
            ...
            for i in range(len(w.nodes) - 1):
                ...
                if n1 in self.noeuds and n2 in self.noeuds:
                    ...
                    # appel à Haversine
                    dist = distance(self.noeuds[n1][0], self.noeuds[n1][1], self.noeuds[n2][0], self.noeuds[n2][1])
                    # création de l'arête dans aretes.txt format : (départ, arrivée, distance)
                    self.aretes.append((n1, n2, dist))
...
aretes_df[['u_mapped', 'v_mapped', 'length']].to_csv("data/aretes.txt", sep=' ', index=False, header=False)
...
noeuds_df.to_csv("data/noeuds.txt", sep=' ', index=False, header=False)