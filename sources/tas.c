...
tas_binaire_t* tas_create(int capacity) {
    ...
    tas->size = 0;
    tas->donnees = malloc(capacity * sizeof(element_tas_t));
    return tas;
}

void tas_destroy(tas_binaire_t * tas) {
    if(tas != NULL) {
        if(tas->donnees != NULL) free(tas->donnees);
        free(tas);
    }
}
...
void tas_ajout(tas_binaire_t * tas, int sommet, double score, double cout_reel) {
    ...
    int i = tas->size;
    tas->donnees[i].sommet = sommet;
    tas->donnees[i].score = score;
    tas->donnees[i].cout_reel = cout_reel;
    tas->size++;
    ...
    while (i > 0) {
        ...
        if (tas->donnees[i].score < tas->donnees[parent].score) {
            element_tas_t temp = tas->donnees[i];
            tas->donnees[i] = tas->donnees[parent];
            tas->donnees[parent] = temp;
...
element_tas_t tas_extraire_min(tas_binaire_t * tas) {
    ...
    element_tas_t racine = tas->donnees[0]; // comme min à la racine on peut le sauvegarder
    tas->size--; // on baisse la taille du tas
    tas->donnees[0] = tas->donnees[tas->size]; // on place le tout dernier élément à la racine de l'arbre pour boçucher le trou
    ...
    while (1) {
        ...
        if (gauche < tas->size && tas->donnees[gauche].score < tas->donnees[min].score)
            min = gauche;
        if (droit < tas->size && tas->donnees[droit].score < tas->donnees[min].score)
            min = droit;
        // echange
        if (min != i) {
            element_tas_t temp = tas->donnees[i];
            tas->donnees[i] = tas->donnees[min];
            tas->donnees[min] = temp;
            i = min;
        } else {
